...
const Task = (props) => {
  ...
  const [totalTime, setTotalTime] = useState(
    props.task.dates.reduce((acc, date) => acc + date.time, 0)
  )
  ...
  const updateTask = (name, time, date, oldCategory, newCategory) => {
    ...
    if (time) {
      ...
      const currentDateIndex = updatedTask.dates.findIndex(
      ...
      if (time > 0) {
        ...
        //! If time passed in is -1, then DELETE that date.
    ...
    const tasks = JSON.parse(localStorage.getItem('tasks'))
    tasks.splice(
      tasks.findIndex((task) => task.id === props.task.id),
      1,
      updatedTask
    )
    localStorage.setItem('tasks', JSON.stringify(tasks))
  ...
  const toggleTimer = async () => {
    ...
    if (!timerRunning) {
      ...
      updateTask(
        undefined,
        props.task.dates[0]?.date === getToday()
          ? runningTime + props.task.dates[0].time
          : runningTime
      )
  ...
  const handleDeleteTaskClick = () => {
    if (props.task.name.toLowerCase() !== 'divider') {
      props.setModalData({
        title: 'Delete Task',
        message: () => (
          <>
            <p>
              Are you sure want to delete task "{props.task.name}" and{' '}
              <span class='warning'>ALL</span> records of your time spent on it?
            </p>
  ...
  const deleteTask = () => {
    ...
    tasks
      .filter((task) => task.category === props.task.category)
      .forEach((task) => {
  ...
  const editTask = () => {
    const newName = window.prompt(
      `Enter a new name for '${props.task.name}'.`,
      props.task.name
    )
  ...
  const editTaskCategory = () => {
    ...
    const newCategory = window.prompt(
      `Enter a category for '${props.task.name}'.`,
      oldCategory.slice(0, 1).toUpperCase() + oldCategory.slice(1)
    )
  ...
  //// Task Reordering
  const sortMode =
    props.currentCategory === 'All' ? 'sortPosition' : 'categorySort'
  const reorder = (direction) => {
    ...
    if (direction === 'up') {
      ...
      //If sorting down, do the opposite of above...
  ...
  return (
    <li
      className={`task ${timerRunning ? 'running' : ''} ${
        props.currentCategory === 'All' ||
        props.task.category === props.currentCategory ||
        (props.currentCategory === 'Uncategorized' &&
          props.task.category === '')
          ? ''
          : 'display-none'
      } ${
        props.task.categorySort ===
        props.tasks
          .filter(
            (task) =>
              task.category === props.currentCategory ||
              (task.category === '' &&
                props.currentCategory === 'Uncategorized')
          )
          .reduce((acc, curr) => {
    ...
    >
      ...
      <div className={`task-name text-shadow`}>
        ...
        <div className='icon-buttons'>
          <div className='icon-button-wrapper hover-popup-wrapper'>
            <span className='hover-popup text-shadow'>{`${
              addTimeFormCollapsed ? 'View/Edit History' : 'Close History'
            }`}</span>
            <MdHistoryToggleOff
              className={`edit-time-btn icon-button icon-shadow ${
                showAddTimeForm ? 'edit-time-btn-form-visible' : ''
              }`}
              onClick={toggleAddTimeForm}
            />
          </div>
          <div className='icon-button-wrapper hover-popup-wrapper'>
            <span className='hover-popup text-shadow'>Edit Name</span>
            <MdEdit
              className='edit-btn icon-button icon-shadow'
              onClick={editTask}
            />
          </div>
          <div className='icon-button-wrapper hover-popup-wrapper'>
          ...
          <div className='icon-button-wrapper hover-popup-wrapper delete-btn-wrapper'>
            ...
            <MdDelete
              className='delete-btn icon-button icon-shadow'
      ...
      <div className={`task-info`}>
        ...
        <div className='sort-btns'>
          ...
          <MdExpandLess
            className='sort-btn'
            onClick={props.task[sortMode] > 1 ? () => reorder('up') : null}
          />
          {/* Sort Down */}
          <MdExpandMore
            className='sort-btn'
            onClick={
              props.currentCategory === 'All'
                ...
                : props.task[sortMode] <
                  JSON.parse(localStorage.getItem('tasks')).filter(
                    (task) => props.currentCategory === task.category
                  ).length
                ? () => reorder('down')
                : null
            }
          ...
          <div
            className={`icon-button-wrapper hover-popup-wrapper delete-btn-wrapper divider-only divider-delete`}
          >
        ...
        {/* //// TASK TIME */}
        <div
          className='task-time text-shadow hover-popup-wrapper'
          onClick={toggleTimeframe}
        >
          <span className='hover-popup text-shadow'>{`${
            mainTimeIsGrandTotal
              ? 'All Time. Click to show Today.'
              : 'Today. Click to show All Time.'
          }`}</span>

          <p className=''>
            <span className='timeframe-toggle-btn-big'>
              {mainTimeIsGrandTotal ? (
                <MdAllInclusive className='icon-shadow-deep' />
              ) : (
                <MdCalendarToday className='icon-shadow-deep' />
              )}
            </span>
        ...
        <div className='task-buttons'>
          <button
            className={`task-btn btn ${timerRunning ? 'running' : ''}`}
            onClick={toggleTimer}
          >
      ...
      {/*//// Add Time Form  */}
      <div
        className={`slide-able add-time-component-wrapper ${
          showAddTimeForm && !addTimeFormAnimationDone
            ? 'reveal'
            : showAddTimeForm
            ? 'revealed'
            : addTimeFormCollapsed ||
              (!showAddTimeForm && addTimeFormAnimationDone)
            ? 'hidden'
            : 'collapse'
        }`}
